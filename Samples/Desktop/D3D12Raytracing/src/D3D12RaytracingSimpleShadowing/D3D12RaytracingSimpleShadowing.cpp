...
void D3D12RaytracingSimpleLighting::InitializeScene()
{
    ...
    // Setup lights.
    {
        ...
        lightAmbientColor = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
...
void D3D12RaytracingSimpleLighting::CreateDescriptorHeap()
{
    ...
    descriptorHeapDesc.NumDescriptors = 1065 * 2 + 1 + 2; 
...
void loadSceneNode(const aiScene *rootscene, const struct aiScene *sc, const struct aiNode* nd);
...
bool fastLoadScene(const char* path)
{
	std::string fastLoadFileName = path;
	fastLoadFileName += ".fastload";

	std::ifstream inFastLoadFileName(fastLoadFileName, std::ios::binary);
	if (!inFastLoadFileName.good())
		return false;


	int32_t nMeshes;
	inFastLoadFileName.read((char*)&nMeshes, sizeof(nMeshes));
	assert(nMeshes >= 0);
	for (int32_t i = 0; i < nMeshes; ++i)
	{
		Mesh mesh;
		int32_t nVertices, nIndices;
		inFastLoadFileName.read((char*)&nVertices, sizeof(nVertices));
		inFastLoadFileName.read((char*)&nIndices, sizeof(nIndices));
		mesh.vertices.resize(nVertices);
		mesh.indices.resize(nIndices);
		inFastLoadFileName.read((char*)&mesh.indices[0], sizeof(Index) * nIndices);
		inFastLoadFileName.read((char*)&mesh.vertices[0], sizeof(Vertex) * nVertices);

		g_Mesh.meshes.emplace_back(mesh);
	}
...
void saveFastLoadScene(const char* path)
{
	std::string fastLoadFileName = path;
	fastLoadFileName += ".fastload";

	std::ofstream outFastLoadFileName(fastLoadFileName, std::ios::binary);
	if (!outFastLoadFileName.good())
		return;


	int32_t nMeshes = g_Mesh.meshes.size();
	outFastLoadFileName.write((char*)&nMeshes, sizeof(nMeshes));
	assert(nMeshes >= 0);
	for (auto& mesh : g_Mesh.meshes)
	{
		int32_t nVertices = mesh.vertices.size(), nIndices = mesh.indices.size();
		outFastLoadFileName.write((char*)&nVertices, sizeof(nVertices));
		outFastLoadFileName.write((char*)&nIndices, sizeof(nIndices));
		outFastLoadFileName.write((char*)&mesh.indices[0], sizeof(Index) * nIndices);
		outFastLoadFileName.write((char*)&mesh.vertices[0], sizeof(Vertex) * nVertices);
		
	}

	outFastLoadFileName.close();
}

void loadScene(const char* path)
{
	if (fastLoadScene(path))
		return;
	...
	for (unsigned int i = 0u; i < scene->mNumMeshes; ++i)
	{
		...
		assert(mesh->HasPositions() && mesh->HasNormals() && "mesh must have positions and normals");
		//assert(mesh->mNumVertices == mesh->mNum);

		for (unsigned int vIdx = 0; vIdx < mesh->mNumVertices; ++vIdx)
		{
			Vertex v;

			aiVector3D pos = mesh->mVertices[vIdx];			v.position = XMFLOAT3(pos[0], pos[1], pos[2]);
			aiVector3D n   = mesh->mNormals[vIdx];			v.normal   = XMFLOAT3(n[0], n[1], n[2]);
		...
		g_Mesh.meshes.emplace_back(outMesh);
	}

	saveFastLoadScene(path);

	//loadSceneNode(scene, scene, scene->mRootNode);
}

void loadSceneNode(const aiScene *rootscene, const aiScene *sc, const aiNode* nd)
{
	unsigned int i;
	unsigned int n = 0, t;
	...
	for (; n < nd->mNumMeshes; ++n) {
		const struct aiMesh* mesh = rootscene->mMeshes[nd->mMeshes[n]];
		...
		for (t = 0; t < mesh->mNumFaces; ++t) {
			...
			case 1:  face_mode = FACEMODE_POINT; break;
			case 2:  face_mode = FACEMODE_LINE; break;
			case 3:  face_mode = FACEMODE_TRIANGLE; break;
			default: face_mode = FACEMODE_POLYGON; break;
			}
	...
	for (n = 0; n < nd->mNumChildren; ++n) {
		loadSceneNode(rootscene, sc, nd->mChildren[n]);
	}
}
...
void D3D12RaytracingSimpleLighting::BuildGeometry()
{
	...
	printf(" Number of vertices =   %i\n", g_Mesh.meshes[0].vertices.size());
	printf(" Number of indices  =   %i\n", g_Mesh.meshes[0].indices.size());
	...
	for(auto& mesh : g_Mesh.meshes)
	{
		...
		D3DMesh d3dMesh{};
		AllocateUploadBuffer(device, mesh.indices.data(), mesh.indices.size() * sizeof(Index), &d3dMesh.indexBuffer.resource);
		AllocateUploadBuffer(device, mesh.vertices.data(), mesh.vertices.size() * sizeof(Vertex), &d3dMesh.vertexBuffer.resource);

		// Vertex buffer is passed to the shader along with index buffer as a descriptor table.
		// Vertex buffer descriptor must follow index buffer descriptor in the descriptor heap.
		UINT descriptorIndexIB = CreateBufferSRV(&d3dMesh.indexBuffer, mesh.indices.size(), sizeof(Index));
		UINT descriptorIndexVB = CreateBufferSRV(&d3dMesh.vertexBuffer, mesh.vertices.size(), sizeof(Vertex));
		m_d3dMeshes.emplace_back(d3dMesh);
...
void D3D12RaytracingSimpleLighting::BuildAccelerationStructures()
{
	...
	for(auto& d3dMesh : m_d3dMeshes)
	{
		D3D12_RAYTRACING_GEOMETRY_DESC geometryDesc = {};
		geometryDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
		geometryDesc.Triangles.IndexBuffer = d3dMesh.indexBuffer.resource->GetGPUVirtualAddress();
		geometryDesc.Triangles.IndexCount = static_cast<UINT>(d3dMesh.indexBuffer.resource->GetDesc().Width) / sizeof(Index);
		geometryDesc.Triangles.IndexFormat = DXGI_FORMAT_R32_UINT;
		geometryDesc.Triangles.Transform = 0;
		geometryDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
		geometryDesc.Triangles.VertexCount = static_cast<UINT>(d3dMesh.vertexBuffer.resource->GetDesc().Width) / sizeof(Vertex);
		geometryDesc.Triangles.VertexBuffer.StartAddress = d3dMesh.vertexBuffer.resource->GetGPUVirtualAddress();
...
void D3D12RaytracingSimpleLighting::OnUpdate()
{
    ...
    {
		float secondsToRotateAround = 4.0f;
		float angleToRotateBy = -360.0f * (elapsedTime / secondsToRotateAround);
		XMMATRIX rotate = XMMatrixRotationY(XMConvertToRadians(angleToRotateBy));
...
void D3D12RaytracingSimpleLighting::DoRaytracing()
{
    ...
    auto SetCommonPipelineState = [&](auto* descriptorSetCommandList)
    {
        ...
        commandList->SetComputeRootDescriptorTable(GlobalRootSignatureParams::VertexBuffersSlot, m_d3dMeshes[0].indexBuffer.gpuDescriptorHandle);
...
void D3D12RaytracingSimpleLighting::ReleaseDeviceDependentResources()
{
	...
	for (auto& d3dMesh : m_d3dMeshes)
	{
		d3dMesh.indexBuffer.resource.Reset();
		d3dMesh.vertexBuffer.resource.Reset();
	}
